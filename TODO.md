...
- **Statusline — adopt `/lab`** · LOW
  `statusLine` in `~/.claude/settings.json` points at `~/.claude/statusline-command.sh`, not the plugin. Wanted: the status line set through `/lab`, painting, with `/lab` `status` reporting healthy.
