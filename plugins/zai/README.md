...
```sh
npx -y @v1nvn/zai@0.24.0                       # ZAI_AUTH_TOKEN → api.z.ai
npx -y @v1nvn/zai@0.24.0 --auth-token TOKEN    # on the command line (visible in ps)
npx -y @v1nvn/zai@0.24.0 --auth-token=TOKEN    # = form works too (zsh quoting-safe)
npx -y @v1nvn/zai@0.24.0 --base-url https://open.bigmodel.cn
```
...
/zai:usage
  └─ UserPromptExpansion hook (hooks/hooks.json)
       └─ npx -y @v1nvn/za@0.24.0i@<version> --hook
