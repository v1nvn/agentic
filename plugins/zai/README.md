...
```sh
npx -y @v1nvn/zai@0.25.0                       # ZAI_AUTH_TOKEN → api.z.ai
npx -y @v1nvn/zai@0.25.0 --auth-token TOKEN    # on the command line (visible in ps)
npx -y @v1nvn/zai@0.25.0 --auth-token=TOKEN    # = form works too (zsh quoting-safe)
npx -y @v1nvn/zai@0.25.0 --base-url https://open.bigmodel.cn
```
