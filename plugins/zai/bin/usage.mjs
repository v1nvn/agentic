 ...
 * and authenticates with ANTHROPIC_AUTH_TOKEN. Fetches model usage, tool usage, and
 * quota limits, then renders a single plain-text usage report (see format.mjs).
 */
...
const fetchJson = (apiUrl, label, appendQueryParams = true, postProcessor = null) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          let out = json.data || json;
          if (postProcessor && json.data) {
            out = postProcessor(json.data);
          }
          resolve(out);
        } catch (e) {
          reject(new Error(`[${label}] could not parse response: ${e.message}`));
        }
      });
    });
...
const run = async () => {
  const [model, tool, quota] = await Promise.all([
    fetchJson(modelUsageUrl, 'Model usage'),
    fetchJson(toolUsageUrl, 'Tool usage'),
    fetchJson(quotaLimitUrl, 'Quota limit', false, processQuotaLimit),
  ]);
  console.log(render({ platform, model, tool, quota }));
};
