...
in the owner's terminal, nothing written:
    ...
    ! npx -y @v1nvn/statusline@0.22.0 configure --model block --bar gauge --fallback=default --dry-run
...
write prints `configured — live on the next paint`, plain text:
    ...
    npx -y @v1nvn/statusline@0.22.0 configure --model block --bar gauge --fallback=default
...
layout does not name is an error:
    ...
    npx -y @v1nvn/statusline@0.22.0 configure --layout '{model effort} {cwd branch} {bar tokens cache} {style}' --style dots --fallback=default
...
with `--force` added to the configuration flags:
    ...
    npx -y @v1nvn/statusline@0.22.0 configure --model block --bar gauge --fallback=default --force
