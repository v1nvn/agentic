#!/bin/sh
# UserPromptExpansion hook for /md:send. Encodes the previous assistant reply into a
# Markdown-Viewer #share= URL, opens it, then BLOCKS the command from reaching the
# model → zero tokens. The "Sent:" URL is returned as the block `reason`.
#
...
msg="$("$bin/last-reply" "$@" | "$bin/md-send" - 2>&1)" \
    || msg="send failed: $msg"
