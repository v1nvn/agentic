...
```sh
npx -y @v1nvn/zai@0.28.0        # GLM Coding Plan usage report
npx -y @v1nvn/tokens@0.28.0     # token usage + cache hit rate from local transcripts
npx -y @v1nvn/rm@0.28.0         # last reply → reMarkable (or a file: npx -y @v1nvn/rm@0.28.0 reply.md)
npx -y @v1nvn/md@0.28.0         # last reply → Markdown-Viewer (--view for read-only; or a file: npx -y @v1nvn/md@0.28.0 reply.md)
```
...
```sh
npx -y @v1nvn/statusline@0.28.0 configure                          # the wizard — bare, on a TTY
npx -y @v1nvn/statusline@0.28.0 configure --theme lean             # a theme write
npx -y @v1nvn/statusline@0.28.0 configure --theme lean --bar gauge # a theme plus one swap
npx -y @v1nvn/statusline@0.28.0 preview --theme rich --plain       # chat-safe sketch, nothing written
npx -y @v1nvn/statusline@0.28.0 catalog                            # themes block, then one line per item
npx -y @v1nvn/statusline@0.28.0 restore                            # both keys back to their pre-lab values
...
`npx -y @v1nvn/statusline@0.28.0 configure` in a terminal, where the wizard
