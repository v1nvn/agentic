...
import { readDeclarations, readDefaults } from './gallery.js';
...
export async function createWizard(
  ...
  const stdin = readFileSync(options.payloadPath, 'utf8');
    ...
    deps.render(
      [
        ...
        ...focused.alternatives.map(
          (alt, at) => `  ${alt === current[focus] ? '*' : ' '} ${samples[at]}`,
        ),
        '',
        ...rows,
        '',
  ...
  async function finish(keys: AsyncGenerator<string>): Promise<WizardOutcome> {
